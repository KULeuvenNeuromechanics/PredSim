...
default_ratio.fingertip_elbow = 1/4;
default_ratio.elbow_shoulder = 1/8;
default_ratio.shoulder_width = 1/4;
default_ratio.hip_knee = 1/4;
default_ratio.knee_ground = 1/4;
default_ratio.foot_length = 1/7;

%% get user inputs
% user_height = ;
% user_ratio.fingertip_elbow = ;
...
% foot
dim_foot.length = default_ratio.foot_length*sf_foot;
dim_foot.height = dim_foot.length/4;
dim_foot.width = dim_foot.length/3;
% lower leg
dim_low_leg.length = default_ratio.knee_ground*sf_low_leg - dim_foot.height;
dim_low_leg.width = dim_low_leg.length/3;
% upper leg
dim_upp_leg.length = default_ratio.hip_knee*sf_upp_leg;
dim_upp_leg.width = dim_upp_leg.length/3;
% upper arm
dim_upp_arm.length = default_ratio.elbow_shoulder*sf_upp_arm;
dim_upp_arm.width = dim_upp_arm.length/3;
% hand
dim_hand.length = 1/10;
dim_hand.width = dim_hand.length*0.6;
% lower arm
dim_low_arm.length = (default_ratio.fingertip_elbow*sf_low_arm - dim_hand.length);
...
dim_trs.shwidth = default_ratio.shoulder_width*sf_shoulder;
...
rectangle('Position',[-1/2,0,1,1],'EdgeColor',ink_colour)
diam = 1.2;
rectangle('Position',[-diam/2,0,diam,diam],'Curvature',1,'EdgeColor',ink_colour)
...
r1 = fill(pos(1,:)',pos(2,:)',colr);
r1.FaceAlpha = 0;
r1.LineWidth = 1;
r1.EdgeColor = colr;

r2 = fill(-pos(1,:)',pos(2,:)',colr);
r2.FaceAlpha = 0;
r2.LineWidth = 1;
r2.EdgeColor = colr;
...
axes(ax)
r1 = rectangle('Position',[-1/12,1-1/6,1/6,1/6],'Curvature',0.9);
r1.EdgeColor = colr1;
% r1.FaceColor = colr1;
...
pos = [p1,p2,p2,p1];
pos(1,3:4) = -pos(1,3:4);
r3 = fill(pos(1,:)',pos(2,:)',colr2);
r3.EdgeColor = colr1;

r2 = rectangle('Position',[-face_w/2,1-1/8,face_w,face_h],'Curvature',0.8);
r2.EdgeColor = colr1;
r2.FaceColor = colr2;
