...
for m = 1:N_muscles
    [Hilldiff(m),FT(m),Fce(m),Fpass(m),Fiso(m),vMmax(m),massM(m)] = ...
        ...
        model_info.muscle_info.shift(m),S.misc.constant_pennation_angle,...
        S.misc.dampingCoefficient,model_info.muscle_info.muscle_stiffness(m),...
        model_info.muscle_info.muscle_strength(m));
end
...
for m = 1:N_muscles
    [lM(m),lMtilde(m),lT(m)] = FiberLength_TendonForce_tendon(FTtilde(m),...
        ...
        model_info.muscle_info.aTendon(m),model_info.muscle_info.shift(m),S.misc.constant_pennation_angle);
end
...
for m = 1:N_muscles
    [vM(m),vMtilde(m),vT(m)] = FiberVelocity_TendonForce_tendon(FTtilde(m),...
        ...
        vMT(m),model_info.muscle_info.aTendon(m),model_info.muscle_info.shift(m),S.misc.constant_pennation_angle);
end
