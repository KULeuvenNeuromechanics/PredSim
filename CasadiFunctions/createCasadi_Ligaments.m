function [f_ligamentMoment] = createCasadi_Ligaments(S,model_info)
% --------------------------------------------------------------------------
...
% OUTPUT:
%   - f_ligamentMoment -
%   * function for angle-moment of ligaments
%
% 
...
if model_info.ligament_info.NLigament > 0
    ...
    for i=idx_coord_single
        ...
        for j=idx_lig'
            ...
            l_lig_SX = model_info.ligament_info.polyFit.DummySamples.lMT(:,j);
            d_lig = model_info.ligament_info.polyFit.DummySamples.dM(:,j,i);
            F_lig_j = f_stiffness(CSA,ls,l_lig_SX);
    ...
    idx_coord_multi = find(sum(model_info.ligament_info.ligament_spanning_multi_coord,1));
    ...
    for j=idx_multi_coord'
        ...
        CSA = model_info.ligament_info.parameters(j).cross_section_area;
        ls = model_info.ligament_info.parameters(j).slack_length;
        F_lig_SX(i) = f_stiffness(CSA,ls,l_lig_SX(i));
    end
    ...
    % Jacobian of lengths to angles (i.e. moment arms), multiplied by forces
    ...
    M_lig_MX = MX(model_info.ligament_info.NLigament,1);
end
