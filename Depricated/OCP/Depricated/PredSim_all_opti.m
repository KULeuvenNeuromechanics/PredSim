...
elseif IGsel == 2 % Data-informed initial guess
    if IGm == 1 % Data from average walking motion
        ...
        guess = getGuess_DI_opti(Qs_walk,nq,N,time_IC,NMuscle,jointi,scaling,...
            v_tgt,d);   
    elseif IGm == 2 % Data from average runing motion    
        time_IC = [Qs_run.time(1),Qs_run.time(end)];
        guess = getGuess_DI_opti(Qs_run,nq,N,time_IC,NMuscle,jointi,scaling,...
            v_tgt,d);
...
if solveProblem
    ...
    % Time derivative of muscle-tendon forces (states) at mesh points
    dFTtilde = opti.variable(NMuscle, N);
    opti.subject_to(bounds.dFTtilde.lower'*ones(1,N) < dFTtilde < bounds.dFTtilde.upper'*ones(1,N));
    opti.set_initial(dFTtilde, guess.dFTtilde');
    % Time derivative of Qdots (states) at mesh points
    A = opti.variable(nq.all, N);
    opti.subject_to(bounds.Qdotdots.lower'*ones(1,N) < A < bounds.Qdotdots.upper'*ones(1,N));
    opti.set_initial(A, guess.Qdotdots');       
    % Arm excitations
    ...
    dFTtilde_nsc = dFTtilde.*scaling.dFTtilde;
    vA_nsc = vA.*scaling.vA;
    A_nsc = A.*(scaling.Qdotdots'*ones(1,N));    
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for k=1:N
        ...
        % States
        ak = a(:,k); FTtildek = FTtilde(:,k); FTtildek_nsc = FTtilde_nsc(:,k);
        Xk = X(:,k); Xk_nsc = X_nsc(:,k); a_ak = a_a(:,k);      
        ak_colloc = [ak a_col(:,(k-1)*d+1:k*d)]; 
        FTtildek_colloc_nsc = [FTtildek_nsc FTtilde_col_nsc(:,(k-1)*d+1:k*d)];
        FTtildek_colloc = [FTtildek FTtilde_col(:,(k-1)*d+1:k*d)];
        Xk_colloc_nsc = [Xk_nsc X_col_nsc(:,(k-1)*d+1:k*d)];
        Xk_colloc = [Xk X_col(:,(k-1)*d+1:k*d)];
        a_ak_colloc = [a_ak a_a_col(:,(k-1)*d+1:k*d)];
        % Controls
        dFTtildek = dFTtilde(:,k); dFTtildek_nsc = dFTtilde_nsc(:,k); 
        vAk = vA(:,k); vAk_nsc = vA_nsc(:,k);
        Ak = A(:,k); Ak_nsc = A_nsc(:,k); 
        e_ak = e_a(:,k);
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%                         
        ...
        [Hilldiffk,FTk,Fcek,Fpassk,Fisok,vMmaxk,massMk] = ...
            f_forceEquilibrium_FtildeState_all_tendon(ak,FTtildek_nsc,...
                dFTtildek_nsc,lMTk_lr,vMTk_lr,tensions,aTendon,shift); 
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % Get metabolic energy rate if in the cost function   
        if W.E ~= 0    
            % Get muscle fiber lengths
            [~,lMtildek] = f_FiberLength_TendonForce_tendon(...
                FTtildek_nsc,lMTk_lr,aTendon,shift); 
            % Get muscle fiber velocities
            [vMk,~] = f_FiberVelocity_TendonForce_tendon(...
                FTtildek_nsc,dFTtildek_nsc,lMTk_lr,vMTk_lr,aTendon,shift);
        ...
        for j=1:d
            ...
            xp_nsc          = Xk_colloc_nsc*C(:,j+1);
            FTtildep_nsc    = FTtildek_colloc_nsc*C(:,j+1);
            ap              = ak_colloc*C(:,j+1);
            a_ap            = a_ak_colloc*C(:,j+1);
            ...
            opti.subject_to((h*dFTtildek_nsc - FTtildep_nsc)./scaling.FTtilde' == 0);
            % Skeleton dynamics (implicit formulation)               
            qdot_colloc_nsc = Xk_colloc_nsc(2:2:end,j+1);
            qp_nsc = xp_nsc(1:2:end);
            opti.subject_to((h*qdot_colloc_nsc - qp_nsc)./scaling.QsQdots(1:2:end)' == 0)
            qdotp_nsc = xp_nsc(2:2:end);            
            opti.subject_to((h*Ak_nsc - qdotp_nsc)./scaling.QsQdots(2:2:end)' == 0)  
            % Arm activation dynamics (explicit formulation)  
            dadt = f_ArmActivationDynamics(e_ak,a_ak_colloc(:,j+1)');
            opti.subject_to((h*dadt - a_ap)./scaling.a_a == 0)   
            % Add contribution to the quadrature function
            if W.E == 0
            J = J + 1/(dist_trav_tot)*(...
                W.A*B(j+1)      *(f_J92(ak_colloc(:,j+1)'))*h + ...
        ...
        opti.subject_to(Tk(ground_pelvisi,1) == 0);
        ...
        opti.subject_to(Tk(jointi.hip_flex.l,1)-(T_hip_flex_l + ...
            Tau_passk.hip.flex.l - 0.1*Xk_nsc(jointi.hip_flex.l*2,1)) == 0);
        % Hip flexion, right
        Ft_hip_flex_r   = FTk(mai(1).mus.r',1);
        T_hip_flex_r    = f_T27(MA.hip_flex.r,Ft_hip_flex_r);
        opti.subject_to(Tk(jointi.hip_flex.r,1)-(T_hip_flex_r + ...
            Tau_passk.hip.flex.r - 0.1*Xk_nsc(jointi.hip_flex.r*2,1)) == 0);
        % Hip adduction, left
        Ft_hip_add_l    = FTk(mai(2).mus.l',1);
        T_hip_add_l     = f_T27(MA.hip_add.l,Ft_hip_add_l);
        opti.subject_to(Tk(jointi.hip_add.l,1)-(T_hip_add_l + ...
            Tau_passk.hip.add.l - 0.1*Xk_nsc(jointi.hip_add.l*2,1)) == 0);
        % Hip adduction, right
        Ft_hip_add_r    = FTk(mai(2).mus.r',1);
        T_hip_add_r     = f_T27(MA.hip_add.r,Ft_hip_add_r);
        opti.subject_to(Tk(jointi.hip_add.r,1)-(T_hip_add_r + ...
            Tau_passk.hip.add.r - 0.1*Xk_nsc(jointi.hip_add.r*2,1)) == 0);
        % Hip rotation, left
        Ft_hip_rot_l    = FTk(mai(3).mus.l',1);
        T_hip_rot_l     = f_T27(MA.hip_rot.l,Ft_hip_rot_l);
        opti.subject_to(Tk(jointi.hip_rot.l,1)-(T_hip_rot_l + ...
            Tau_passk.hip.rot.l - 0.1*Xk_nsc(jointi.hip_rot.l*2,1)) == 0);
        % Hip rotation, right
        Ft_hip_rot_r    = FTk(mai(3).mus.r',1);
        T_hip_rot_r     = f_T27(MA.hip_rot.r,Ft_hip_rot_r);
        opti.subject_to(Tk(jointi.hip_rot.r,1)-(T_hip_rot_r + ...
            Tau_passk.hip.rot.r - 0.1*Xk_nsc(jointi.hip_rot.r*2,1)) == 0);
        % Knee, left
        Ft_knee_l       = FTk(mai(4).mus.l',1);
        T_knee_l        = f_T13(MA.knee.l,Ft_knee_l);
        opti.subject_to(Tk(jointi.knee.l,1)-(T_knee_l + ...
            Tau_passk.knee.l - 0.1*Xk_nsc(jointi.knee.l*2,1)) == 0);   
        % Knee, right
        Ft_knee_r       = FTk(mai(4).mus.r',1);
        T_knee_r        = f_T13(MA.knee.r,Ft_knee_r);
        opti.subject_to(Tk(jointi.knee.r,1)-(T_knee_r + ...
            Tau_passk.knee.r - 0.1*Xk_nsc(jointi.knee.r*2,1)) == 0);
        % Ankle, left
        Ft_ankle_l      = FTk(mai(5).mus.l',1);
        T_ankle_l       = f_T12(MA.ankle.l,Ft_ankle_l);
        opti.subject_to(Tk(jointi.ankle.l,1)-(T_ankle_l + ...
            Tau_passk.ankle.l - 0.1*Xk_nsc(jointi.ankle.l*2,1)) == 0);
        % Ankle, right
        Ft_ankle_r      = FTk(mai(5).mus.r',1);
        T_ankle_r       = f_T12(MA.ankle.r,Ft_ankle_r);
        opti.subject_to(Tk(jointi.ankle.r,1)-(T_ankle_r + ...
            Tau_passk.ankle.r - 0.1*Xk_nsc(jointi.ankle.r*2,1)) == 0);
        % Subtalar, left
        Ft_subt_l       = FTk(mai(6).mus.l',1);
        T_subt_l        = f_T12(MA.subt.l,Ft_subt_l);
        opti.subject_to(Tk(jointi.subt.l,1)-(T_subt_l + ...
            Tau_passk.subt.l - 0.1*Xk_nsc(jointi.subt.l*2,1)) == 0);   
        % Subtalar, right
        Ft_subt_r       = FTk(mai(6).mus.r',1);
        T_subt_r        = f_T12(MA.subt.r,Ft_subt_r);
        opti.subject_to(Tk(jointi.subt.r,1)-(T_subt_r + ...
            Tau_passk.subt.r - 0.1*Xk_nsc(jointi.subt.r*2,1)) == 0);
        % Lumbar extension
        Ft_trunk_ext    = FTk([mai(7).mus.l,mai(7).mus.r]',1);
        T_trunk_ext     = f_T6(MA.trunk_ext,Ft_trunk_ext);
        opti.subject_to(Tk(jointi.trunk.ext,1)-(T_trunk_ext + ...
            Tau_passk.trunk.ext - 0.1*Xk_nsc(jointi.trunk.ext*2,1)) == 0);
        % Lumbar bending
        Ft_trunk_ben    = FTk([mai(8).mus.l,mai(8).mus.r]',1);
        T_trunk_ben     = f_T6(MA.trunk_ben,Ft_trunk_ben);
        opti.subject_to(Tk(jointi.trunk.ben,1)-(T_trunk_ben + ...
            Tau_passk.trunk.ben - 0.1*Xk_nsc(jointi.trunk.ben*2,1)) == 0);
        % Lumbar rotation
        Ft_trunk_rot    = FTk([mai(9).mus.l,mai(9).mus.r]',1);
        T_trunk_rot     = f_T6(MA.trunk_rot,Ft_trunk_rot);
        opti.subject_to(Tk(jointi.trunk.rot,1)-(T_trunk_rot + ...
            Tau_passk.trunk.rot - 0.1*Xk_nsc(jointi.trunk.rot*2,1)) == 0);
        % Torque-driven joint torques for the arms
        % Arms
        opti.subject_to(Tk(armsi,1)/scaling.ArmTau - a_ak + ...
            0.1/scaling.ArmTau*Xk_nsc(armsi*2,1) == 0);
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % Activation dynamics (implicit formulation)
        tact = 0.015;
        tdeact = 0.06;
        ...
        opti.subject_to(0.0081 < f_Jnn2(Tk(calcOr.r,1) - Tk(calcOr.l,1)) < 4);  
        ...
        opti.subject_to(0.0324 < f_Jnn2(Tk(femurOr.r,1) - Tk(handOr.r,1)) < 4);
        opti.subject_to(0.0324 < f_Jnn2(Tk(femurOr.l,1) - Tk(handOr.l,1)) < 4);
        % Origins tibia (transv plane) at minimum 11 cm from each other.   
        opti.subject_to(0.0121 < f_Jnn2(Tk(tibiaOr.r,1) - Tk(tibiaOr.l,1)) < 4);
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if analyseResults
    ...
    dist_trav_opt = q_mesh_col_opt(end,jointi.pelvis.tx)*...
        scaling.Qs(jointi.pelvis.tx) - q_mesh_col_opt(1,jointi.pelvis.tx)*...
...
if decomposeCost
    ...
    if saveResults
        ...
        % Save data
        save([pathresults,'/',namescript,'/Results_all_opti.mat'],...
            'Results_all');
    end    
end
end
