...
elseif IGsel == 2 % Data-informed initial guess
    if IGm == 1 % Data from average walking motion
        ...
        guess = getGuess_DI_opti_int(Qs_walk,nq,N,time_IC,NMuscle,jointi,scaling,...
            v_tgt,d);   
    elseif IGm == 2 % Data from average runing motion    
        time_IC = [Qs_run.time(1),Qs_run.time(end)];
        guess = getGuess_DI_opti_int(Qs_run,nq,N,time_IC,NMuscle,jointi,scaling,...
            v_tgt,d);
...
if solveProblem
    ...
    for j=1:d      
        ...
        Tau_passj.knee.l        = f_PassiveMoments(k_pass.knee,...
            theta.pass.knee,Xkj_nsc(jointi.knee.l*2-1,j+1),...
            Xkj_nsc(jointi.knee.l*2,j+1));
        Tau_passj.knee.r        = f_PassiveMoments(k_pass.knee,...
            theta.pass.knee,Xkj_nsc(jointi.knee.r*2-1,j+1),...
            Xkj_nsc(jointi.knee.r*2,j+1));
        ...
        eq_constr{end+1} = (hk*dadtj - a_ap)./scaling.a_a;
        % Add contribution to the quadrature function
        if W.E == 0
        J = J + (...
            W.A*B(j+1)      *(f_J92(akj(:,j+1)'))*hk + ...
            W.ArmE*B(j+1)   *(f_J8(e_ak))*hk +... 
            W.Ak*B(j+1)     *(f_J21(Aj(residuals_noarmsi,j)))*hk + ... 
            W.passMom*B(j+1)*(f_J15(Tau_passj_all))*hk + ...
            W.u*B(j+1)      *(f_J92(vAk))*hk + ...
            W.u*B(j+1)      *(f_J92(dFTtildej(:,j)))*hk + ...                
            W.u*B(j+1)      *(f_J8(Aj(armsi,j)))*hk);
        elseif W.A == 0
        J = J + (...
            W.E*B(j+1)      *(f_J92exp(e_totj,exp_E))/body_mass*hk + ...                
            W.ArmE*B(j+1)   *(f_J8(e_ak))*hk +... 
            W.Ak*B(j+1)     *(f_J21(Aj(residuals_noarmsi,j)))*hk + ... 
            W.passMom*B(j+1)*(f_J15(Tau_passj_all))*hk + ...
            W.u*B(j+1)      *(f_J92(vAk))*hk + ...
        ...
        eq_constr{end+1} = Tj(jointi.hip_flex.l,1)-(T_hip_flex_l + ...
            Tau_passj.hip.flex.l - 0.1*Xkj_nsc(jointi.hip_flex.l*2,j+1));
        % Hip flexion, right
        ...
        eq_constr{end+1} = Tj(jointi.hip_flex.r,1)-(T_hip_flex_r + ...
            Tau_passj.hip.flex.r - 0.1*Xkj_nsc(jointi.hip_flex.r*2,j+1));
        % Hip adduction, left
        ...
        eq_constr{end+1} = Tj(jointi.hip_add.l,1)-(T_hip_add_l + ...
            Tau_passj.hip.add.l - 0.1*Xkj_nsc(jointi.hip_add.l*2,j+1));
        % Hip adduction, right
        ...
        eq_constr{end+1} = Tj(jointi.hip_add.r,1)-(T_hip_add_r + ...
            Tau_passj.hip.add.r - 0.1*Xkj_nsc(jointi.hip_add.r*2,j+1));
        % Hip rotation, left
        ...
        eq_constr{end+1} = Tj(jointi.hip_rot.l,1)-(T_hip_rot_l + ...
            Tau_passj.hip.rot.l - 0.1*Xkj_nsc(jointi.hip_rot.l*2,j+1));
        % Hip rotation, right
        ...
        eq_constr{end+1} = Tj(jointi.trunk.ext,1)-(T_trunk_ext + ...
            Tau_passj.trunk.ext - 0.1*Xkj_nsc(jointi.trunk.ext*2,j+1));
        % Lumbar bending
        ...
        eq_constr{end+1} = Tj(jointi.trunk.ben,1)-(T_trunk_ben + ...
            Tau_passj.trunk.ben - 0.1*Xkj_nsc(jointi.trunk.ben*2,j+1));
        % Lumbar rotation
        ...
        eq_constr{end+1} = Tj(jointi.trunk.rot,1)-(T_trunk_rot + ...
            Tau_passj.trunk.rot - 0.1*Xkj_nsc(jointi.trunk.rot*2,j+1));
        % Torque-driven joint torques for the arms
        % Arms
        eq_constr{end+1} = Tj(armsi,1)/scaling.ArmTau - a_akj(:,j+1) + ...
            0.1/scaling.ArmTau*Xkj_nsc(armsi*2,j+1);
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % Activation dynamics (implicit formulation)            
    ...
    f_coll = Function('f_coll',{tfk,ak,aj,FTtildek,FTtildej,Xk,Xj,a_ak,a_aj,...
        vAk,e_ak,dFTtildej,Aj},{eq_constr,ineq_constr1,ineq_constr2,...
        ineq_constr3,ineq_constr4,ineq_constr5,J});
    f_coll_map = f_coll.map(N,'openmp',4);
    [coll_eq_constr, coll_ineq_constr1, coll_ineq_constr2, coll_ineq_constr3,...
        ...
        a(:,1:end-1), a_col, FTtilde(:,1:end-1), FTtilde_col, X(:,1:end-1), ...
        X_col, a_a(:,1:end-1), a_a_col, vA, e_a, dFTtilde_col, A_col);
...
if analyseResults
    ...
    % Arm activations (1:N)
    a_a_opt_unsc_all = a_a_opt.*repmat(scaling.a_a,size(a_a_opt,1),size(a_a_opt,2));
    ...
    % Qdots
    qdot_col_opt_unsc.rad = qdot_col_opt.*repmat(...
        scaling.Qdots,size(qdot_col_opt,1),1); 
    % Convert in degrees
    ...
    for k=1:N     
        for j=1:d 
            ...
            [~,lMtilde_opt_all] = f_FiberLength_TendonForce_tendon(...
                FTtilde_col_opt_unsc(count,:)',full(lMTk_lr_opt_all),aTendon,shift);                
            [vM_opt_all,~] = ...
                f_FiberVelocity_TendonForce_tendon(FTtilde_col_opt_unsc(count,:)',...
                dFTtilde_col_opt_unsc(count,:)',full(lMTk_lr_opt_all),...
                full(vMTk_lr_opt_all),aTendon,shift);   

            if mE == 0 % Bhargava et al. (2004)
                [e_tot_all,~,~,~,~,~] = fgetMetabolicEnergySmooth2004all(...
                    a_col_opt_unsc(count,:)',a_col_opt_unsc(count,:)',full(lMtilde_opt_all),...
...
%             OptSol;   
        ...
        % Save data
        save([pathresults,'/',namescript,'/Results_all_opti_int_NLPSol.mat'],...
            'Results_all');
    end    
end
end
