...
% output folder
S.ResultsFolder     = 'FootHeight_batch2_igExpWalk';

% initial guess based on simulations without exoskeletons
S.IGsel         = 2;        % initial guess identifier (1: quasi random, 2: data-based)
S.IGmodeID      = 1;        % initial guess mode identifier (1 walk, 2 run, 3 prev.solution, 4 solution from /IG/Data folder)
...
BatchH = [0 0.05 0.08 0.1 0.12 0.15 0.2 0.25];
for i=1:length(BatchH)
    % savename
    S.savename   =  ['Sedgeford_Mean_FootHeight_' num2str(i)];
    ...
    S.Constr.FootHeight.Ry_Treshold = BatchH(i); % minimal height of the foot (when velocity constraint is active)
