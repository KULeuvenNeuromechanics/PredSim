...
% output folder
S.ResultsFolder     = 'FootHeight_batch4';
...
BatchVx = [4 5 6 3 2];
for i=1:length(BatchVx)
    % savename
    S.savename   =  ['Sedgeford_Mean_tf_Vx_' num2str(i)];
    S.Constr.ImposeFootHeight = true; % imposing contraint on foot height during swing
    S.Constr.FootHeight.Vx_Treshold = BatchVx(i); % minimal forward velocity of the foot when constraint on footheight is applied
    S.Constr.FootHeight.Ry_Treshold = 0.12; % minimal height of the foot (when velocity constraint is active)
