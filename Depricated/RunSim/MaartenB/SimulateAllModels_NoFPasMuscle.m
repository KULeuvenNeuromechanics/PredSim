...
% output folder
S.ResultsFolder     = 'AllModels_MaartenB_FPas';
...
%% Simulation with offset in passive muscle forces

% selection folder with Casadi Functions
S.CasadiFunc_Folders = 'Sedgeford_Mean_FPasMuscle';
% select folder with polynomials
S.PolyFolder = 'Sedgeford_Mean';
% savename
S.savename   =  'Sedgeford_Mean_free';
% run the optimization
f_PredSim_Gait92(S);     % run the optimization
f_LoadSim_Gait92(S.ResultsFolder,S.savename) % post-proces simulation results

%% Simulation with imposed stride frequency and offset in passive muscle F

FinalTime = [0.3 0.35 0.4];
