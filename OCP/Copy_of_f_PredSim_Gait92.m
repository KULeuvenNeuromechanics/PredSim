...
residualsi = 1:length(fields(IO.coordi));
ground_pelvisi      = model_info.ExtFunIO.jointi.pelvis; % ground-pelvis
...
muscleNames = model_info.ExtFunIO.muscle.params.names;
...
NMuscle = size(model_info.ExtFunIO.muscle.params.params,2);
...
[~,mai] = MomentArmIndices(muscleNames(1:end/2),tl.muscle_spanning_joint_INFO);
...
[IndexLeft,IndexRight,QsInvA,QsInvB,QdotsInvA,...
    QdotsInvB,orderQsOpp] = GetIndexHelper(S,muscle_info.ExtFunIO);
...
lboundsQsk(model_info.ExtFunIO.jointi.pelvis.tx,1) = ...
    bounds.QsQdots_0.lower(2*model_info.ExtFunIO.jointi.pelvis.tx-1);
uboundsQsk = bounds.QsQdots.upper(1:2:end)'*ones(1,N+1);
uboundsQsk(model_info.ExtFunIO.jointi.pelvis.tx,1) = ...
    bounds.QsQdots_0.upper(2*model_info.ExtFunIO.jointi.pelvis.tx-1);
