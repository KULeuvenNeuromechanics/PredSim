...
for j=1:d
    ...
    elseif strcmp(S.metabolicE.model,'Uchida2016')
                % Get metabolic energy rate Bhargava et al. (2004)
        [e_totj,~,~,~,~,~] = f_casadi.getMetabolicEnergySmooth2016all(...
            akj(:,j+1),akj(:,j+1),lMtildej,vMj,Fcej,Fpassj,...
            MuscleMass',pctsts,Fisoj,model_info.mass,S.metabolicE.tanh_b);
          
    elseif strcmp(S.metabolicE.model,'Margaria')
                % Get metabolic energy rate Margaria
        [e_totj,~,~,~,~,~] = f_casadi.getMetabolicEnergyMargaria(...
            akj(:,j+1),akj(:,j+1),lMtildej,vMj,Fcej,Fpassj,...
            MuscleMass',pctsts,Fisoj,model_info.mass,S.metabolicE.tanh_b);
...
for k=1:N
    for j=1:d
        ...
        elseif strcmp(S.metabolicE.model,'Uchida2016')
            [e_tot_all,~,~,~,~,~] = f_casadi.getMetabolicEnergySmooth2016all(...
            a_col_opt_unsc(count,:)',a_col_opt_unsc(count,:)',...
            full(lMtilde_opt_all),...
            full(vM_opt_all),full(Fce_opt_all),full(Fpass_opt_all),...
            MuscleMass',pctsts,full(Fiso_opt_all),model_info.mass,S.metabolicE.tanh_b);
        ...
        elseif strcmp(S.metabolicE.model,'Margaria')
            [e_tot_all,~,~,~,~,~] = f_casadi.getMetabolicEnergyMargaria(...
            a_col_opt_unsc(count,:)',a_col_opt_unsc(count,:)',...
            full(lMtilde_opt_all),...
            full(vM_opt_all),full(Fce_opt_all),full(Fpass_opt_all),...
            MuscleMass',pctsts,full(Fiso_opt_all),model_info.mass,S.metabolicE.tanh_b);
