...
N_arms_dof = model_info.ExtFunIO.nq.arms; %Zero elements in [# muscles crossing each joint]
N_noarms_dof = model_info.ExtFunIO.nq.noarms; %Zero elements in [# muscles crossing each joint]
N_musc_dof = model_info.ExtFunIO.nq.muscleActuated; %Non-zero elements in [# muscles crossing each joint]
...
for i=1:length(e_temp_arms_dof)
    J_temp_arms_dof = J_temp_arms_dof + e_temp_non_musc_dof(i).^2;
end
