function [] = PlotResults_3DSim(ResultsFile,Cs,LegName,PathExpData,varargin)
...
if exist(ResultsFile,'file')
    ...
    joints_ref_sim = {'pelvis_tilt','pelvis_list','pelvis_rotation',...
    ...
    % adapt this code to make it more general. We have to find now the
    % correct cols in the simulations result.
    axes('parent', tab1);

    if boolFirst
        load(PathExpData,'ExperimentalData');
...
%     idx_Qs = [1,2,3,10,11,12,14,16,18,20,21,22,23,27,28,29,31];
    ...
    for i = 1:length(joints_ref)
        subplot(3,6,i)
        x = 1:(100-1)/(size(R.kinematics.Qs,1)-1):100;
        % Experimental data
        if ~(strcmp(joints_ref{i},'mtp_angle')) && boolFirst == 1
            for is = 1:length(RefSNames)
                ...
                if isfield(Qref,subject) && isfield(Qref.(subject),'Qs')
                    ...
                    stepQ = (size(R.kinematics.Qs,1)-1)/(size(meanPlusSTD,1)-1);
                    intervalQ = 1:stepQ:size(R.kinematics.Qs,1);
                    sampleQ = 1:size(R.kinematics.Qs,1);
        ...
        % Simulation results
        x = 1:(100-1)/(size(R.kinematics.Qs,1)-1):100;
        iColSim = find(strcmp(R.colheaders.coordinates,joints_ref_sim{i}));
        hold on;
        if i == length(joints_ref)
            plot(x,R.kinematics.Qs(:,iColSim),'color',Cs,'linewidth',line_linewidth,'DisplayName',LegName);
        else
            plot(x,R.kinematics.Qs(:,iColSim),'color',Cs,'linewidth',line_linewidth);
        end

        % Plot settings
        if boolFirst
            set(gca,'Fontsize',label_fontsize);
            title(joints_ref{i},'Fontsize',label_fontsize,'interpreter','none');
            % Y-axis
            if i == 1 || i == 5 || i == 9 ||i == 13
    ...
    for i = 1:length(joints_ref)
        ...
        if ~(strcmp(joints_ref{i},'mtp_angle')) && boolFirst == 1
            for is = 1:length(RefSNames)
                ...
                if isfield(IDref,subject) && ~isempty(IDref.(subject).mean)
                    ...
                    stepID = (size(R.kinematics.Qs,1)-1)/(size(meanPlusSTD,1)-1);
                    intervalID = 1:stepID:size(R.kinematics.Qs,1);
                    sampleID = 1:size(R.kinematics.Qs,1);
        ...
        hold on;
        if i == length(joints_ref)
            plot(x,R.kinetics.T_ID(:,iColSim),'color',Cs,'linewidth',line_linewidth,'DisplayName',LegName);
        else
            plot(x,R.kinetics.T_ID(:,iColSim),'color',Cs,'linewidth',line_linewidth);
        end
    ...
    plot(xParam,abs(R.metabolics.Bhargava2004.COT),'o','Color',Cs,'MarkerFaceColor',Cs,'DisplayName',LegName);
    ...
    plot(xParam,R.spatiotemp.stride_freq,'o','Color',Cs,'MarkerFaceColor',Cs);
    ylabel('Stride Frequency'); xlabel(xParamLab);
    title('Stride Frequency');
    set(gca,'box','off');
    if boolFirst
        if isfield(ExperimentalData,'StrideFreq') && isfield(ExperimentalData.StrideFreq,RefSNames{1})
            SF_mean = ExperimentalData.StrideFreq.(RefSNames{1}).mean;
            SF_std = ExperimentalData.StrideFreq.(RefSNames{1}).std;
    ...
    subplot(5,2,1)
    plot(x,R.muscles.a(:,iSol),'-','Color',Cs,'LineWidth',lw); hold on; title('Soleus');
    xlabel('% stride'); ylabel('activity');
    set(gca,'box','off');

    subplot(5,2,2)
    plot(x,R.muscles.a(:,iGas),'-','Color',Cs,'LineWidth',lw); hold on; title('Gastrocnemius');
    xlabel('% stride'); ylabel('activity');
    set(gca,'box','off');

    subplot(5,2,3)
    plot(x,R.metabolics.Bhargava2004.Edot_gait(:,iSol),'-','Color',Cs,'LineWidth',lw); hold on;
    xlabel('% stride'); ylabel('Muscle metab power');
    set(gca,'box','off');

    subplot(5,2,4)
    plot(x,R.metabolics.Bhargava2004.Edot_gait(:,iGas),'-','Color',Cs,'LineWidth',lw); hold on;
    xlabel('% stride'); ylabel('Muscle metab power (W)');
    set(gca,'box','off');

    subplot(5,2,5)
    plot(x,R.muscles.lMtilde(:,iSol),'-','Color',Cs,'LineWidth',lw); hold on;
    xlabel('% stride'); ylabel('Norm fiber length');
    set(gca,'box','off');

    subplot(5,2,6)
    plot(x,R.muscles.lMtilde(:,iGas),'-','Color',Cs,'LineWidth',lw); hold on;
    xlabel('% stride'); ylabel('Norm fiber length');
    set(gca,'box','off');

    subplot(5,2,7)
    plot(x,R.metabolics.Bhargava2004.Wdot(:,iSol),'-','Color',Cs,'LineWidth',lw); hold on;
    xlabel('% stride'); ylabel('Wdot');
    set(gca,'box','off');

    subplot(5,2,8)
    plot(x,R.metabolics.Bhargava2004.Wdot(:,iGas),'-','Color',Cs,'LineWidth',lw); hold on;
    xlabel('% stride'); ylabel('Wdot');
    set(gca,'box','off');

    subplot(5,2,9)
    plot(x, R.muscles.FT(:,iSol),'-','Color',Cs,'LineWidth',lw); hold on;
    xlabel('% stride'); ylabel('Norm muscle force');
    set(gca,'box','off');

    subplot(5,2,10)
    plot(x, R.muscles.FT(:,iGas),'-','Color',Cs,'DisplayName',LegName,'LineWidth',lw); hold on;
    xlabel('% stride'); ylabel('Norm muscle force');
    set(gca,'box','off');
    ...
    GRFs = [R.ground_reaction.GRF_r R.ground_reaction.GRF_l]./R.misc.body_weight*100;
    for i=1:6
        subplot(2,3,i);
        l = plot(GRFs(:,i),'-','Color',Cs,'LineWidth',lw); hold on;
        xlabel('% stride');
        set(gca,'box','off');
        if i==1
    ...
    %% Objective function
    axes('parent', tab7);
    labels = R.objective.labels;
    nf = length(labels);
    for i= 1:nf
        subplot(3,4,i)
        l = plot(xParam,R.objective.relativeValues(i),'o','Color',Cs,'MarkerFaceColor',Cs); hold on;
        xlabel(xParamLab);
        title(labels{i});
        set(gca,'box','off');
    end
    subplot(3,4,nf+1)
    l = plot(xParam,abs(sum(R.objective.absoluteValues)),'o','Color',Cs,'MarkerFaceColor',Cs); hold on;
    xlabel(xParamLab);
    title('total cost');
    set(gca,'box','off');
    l.DisplayName = LegName;
    ...
    for i=1:4
        subplot(5,4,i)
        plot(x,R.muscles.a(:,iM(i)),'-','Color',Cs,'LineWidth',lw); hold on; title(mVect{i});
        xlabel('% stride'); ylabel('activity');
        set(gca,'box','off');

        subplot(5,4,i+4)
        plot(x,R.metabolics.Bhargava2004.Edot_gait(:,iM(i)),'-','Color',Cs,'LineWidth',lw); hold on;
        xlabel('% stride'); ylabel('Muscle metab power');
        set(gca,'box','off');

        subplot(5,4,i+8)
        plot(x,R.muscles.lMtilde(:,iM(i)),'-','Color',Cs,'LineWidth',lw); hold on;
        xlabel('% stride'); ylabel('Norm fiber length');
        set(gca,'box','off');

        subplot(5,4,i+12)
        plot(x, R.muscles.FT(:,iM(i)),'-','Color',Cs,'LineWidth',lw); hold on;
        xlabel('% stride'); ylabel('Norm muscle force');
        set(gca,'box','off');
    ...
    subplot(5,4,17)
    plot(x,R.kinetics.T_ID(:,strcmp(R.colheaders.coordinates,'ankle_angle_r')),'-','Color',Cs,'LineWidth',lw); hold on;
    ...
    subplot(5,4,19)
    plot(x,R.kinetics.T_ID(:,strcmp(R.colheaders.coordinates,'subtalar_angle_r')),'-','Color',Cs,'LineWidth',lw); hold on;
    ylabel('subtalar moment [Nm]'); xlabel('% stride');
    set(gca,'box','off');

    subplot(5,4,20)
    l = plot(x,R.kinetics.T_ID(:,strcmp(R.colheaders.coordinates,'mtp_angle_r')),'-','Color',Cs,'LineWidth',lw); hold on;
    ...
    subplot(2,3,1);
    plot(xParam,abs(R.spatiotemp.step_length_l),'o','Color',Cs,'MarkerFaceColor',Cs); hold on;
    ...
    if isfield(R,'StepWidth_COP')
        subplot(2,3,2);
        plot(xParam,R.spatiotemp.step_width_COP,'o','Color',Cs,'MarkerFaceColor',Cs); hold on;
    ...
    subplot(2,3,3);
    plot(xParam,R.spatiotemp.stride_freq,'o','Color',Cs,'MarkerFaceColor',Cs); hold on;
    ...
    subplot(2,3,4);
    plot(xParam,R.spatiotemp.stance_l,'o','Color',Cs,'MarkerFaceColor',Cs); hold on;
    ...
    subplot(2,3,5);
    plot(xParam,R.spatiotemp.swing_l,'o','Color',Cs,'MarkerFaceColor',Cs); hold on;
    ...
    subplot(2,3,6);
    l = plot(xParam,R.spatiotemp.double_support,'o','Color',Cs,'MarkerFaceColor',Cs); hold on;
    ...
    subplot(nr,nc,1);
    plot(xParam,stats.t_proc_total,'o','Color',Cs,'MarkerFaceColor',Cs); hold on;
    set(gca,'box','off');
    ylabel('Total time [s]');
    xlabel(xParamLab);

    subplot(nr,nc,2);
    plot(xParam,stats.iter_count,'o','Color',Cs,'MarkerFaceColor',Cs); hold on;
    set(gca,'box','off');
    ylabel('N iterations []');
    xlabel(xParamLab);

    dt_NLP = stats.t_proc_nlp_f +stats.t_proc_nlp_g +stats.t_proc_nlp_grad_f +stats.t_proc_nlp_jac_g;
    subplot(nr,nc,3);
    ...
    subplot(nr,nc,4);
    plot(xParam,stats.success,'o','Color',Cs,'MarkerFaceColor',Cs); hold on;
    set(gca,'box','off');
    ...
    if isfield(R.S,'ExoDesignBool') && R.S.ExoDesignBool
        ...
        for ij = 1:length(StrVect)
            iJoint = strcmp(R.colheaders.coordinates,StrVect{ij});
